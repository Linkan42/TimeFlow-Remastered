  ...
  try {
    ...
    if (emailFound) {
      ...
      return res.status(404).json({
        error: `User with email ${Email} not found and cannot be deleted`
      });
    }
  } catch (error) {
    return res.status(500).json({
...
app.post("/api/user/UpdateUser", async (req, res) => {
  try {
    ...
    if (emailFound) {
      ...
      return res.status(200).json({
        message: `User with email ${Email} was updated`
      });
    } else {
      return res.status(404).json({
        error: `User with email ${Email} not found and cannot be updated`
      });
    }
  } catch (error) {
    console.log(error);
